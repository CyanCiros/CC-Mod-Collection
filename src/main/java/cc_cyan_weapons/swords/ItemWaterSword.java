...
public class ItemWaterSword extends ItemSword {
	public ItemWaterSword(final ToolMaterial toolMaterial, final String name) {
		super(toolMaterial);
	...
	public ItemStack onItemRightClick(final ItemStack itemStack, final World world, final EntityPlayer entityPlayer) {
		if (Config.waterSwordRightClickEffect) {
			...
			entityPlayer.addPotionEffect(new PotionEffect(Potion.waterBreathing.id, 300, 0));
	...
	private void placeBlock(final World world, final int x, final int y, final int z) {
		for (int vy = -2; vy <= 2; vy++) {
			if (!world.isAirBlock(new BlockPos(x, y + vy - 1, z)) && world.isAirBlock(new BlockPos(x, y + vy, z))) {
				world.setBlockState(new BlockPos(x, y + vy, z), Blocks.water.getDefaultState());
				world.setBlockState(new BlockPos(x, y + vy + 1, z), Blocks.water.getDefaultState());
				break;
			}
		}
	}
