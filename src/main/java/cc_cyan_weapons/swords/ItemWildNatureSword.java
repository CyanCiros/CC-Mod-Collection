...
public class ItemWildNatureSword extends ItemSword {
	public ItemWildNatureSword(final ToolMaterial toolMaterial, final String name) {
		super(toolMaterial);
	...
	public ItemStack onItemRightClick(final ItemStack itemStack, final World world, final EntityPlayer entityPlayer) {
		if (Config.wildNatureSwordRightClickEffect) {
	...
	private void placeBlock(final World world, final int x, final int y, final int z) {
		for (int vy = -2; vy <= 2; vy++) {
			if (!world.isAirBlock(new BlockPos(x, y + vy - 1, z)) && world.isAirBlock(new BlockPos(x, y + vy, z))) {
				world.setBlockState(new BlockPos(x, y + vy, z), Blocks.cactus.getDefaultState());
				world.setBlockState(new BlockPos(x, y + vy + 1, z), Blocks.cactus.getDefaultState());
				break;
			}
		}
	}
	...
	public boolean hitEntity(final ItemStack itemStack, final EntityLivingBase entity1,
		...
		if (Config.wildNatureSwordHitEntityEffect) {
