...
import cc_cyan_weapons.entity.EntityFireArrow;
...
public class ItemFireBow extends ModBow {
	public float arrowDamage;

	public ItemFireBow(final String name, final int maxDamage, final float arrowDamage) {
	...
	public void onPlayerStoppedUsing(final ItemStack itemStack, final World world, final EntityPlayer entityPlayer,
		...
		if (flag || entityPlayer.inventory.hasItem(ModItems.fireArrow)) {
			float a = j / 20.0F;
			a = (a * a + a * 2.0F) / 3.0F;
			...
			final EntityFireArrow entityarrow = new EntityFireArrow(world, entityPlayer, a * 2.0F);
			...
			if (flag) {
				...
				entityPlayer.inventory.consumeInventoryItem(ModItems.fireArrow);
			}
			if (!world.isRemote) {
	...
	public ItemStack onItemRightClick(final ItemStack itemStack, final World world, final EntityPlayer entityPlayer) {
		...
		if (entityPlayer.capabilities.isCreativeMode || entityPlayer.inventory.hasItem(ModItems.fireArrow)) {
	...
	public boolean hitEntity(final ItemStack itemStack, final EntityLivingBase entity1,
		...
		if (Config.fireBowHitEntityEffect) {
			entity1.setFire(9);
		}
