...
import cc_cyan_weapons.entity.EntityMeteoricThunderstormArrow;
...
public class ItemMeteoricThunderstormBow extends ModBow {
	public float arrowDamage;

	public ItemMeteoricThunderstormBow(final String name, final int maxDamage, final float arrowDamage) {
	...
	public void onPlayerStoppedUsing(final ItemStack itemStack, final World world, final EntityPlayer entityPlayer,
		...
		if (flag || entityPlayer.inventory.hasItem(ModItems.meteoricThunderstormArrow)) {
			float a = j / 20.0F;
			a = (a * a + a * 2.0F) / 3.0F;
			...
			final EntityMeteoricThunderstormArrow entityarrow = new EntityMeteoricThunderstormArrow(world, entityPlayer,
					a * 2.0F);
			...
			if (flag) {
				...
				entityPlayer.inventory.consumeInventoryItem(ModItems.meteoricThunderstormArrow);
			}
			if (!world.isRemote) {
	...
	public ItemStack onItemRightClick(final ItemStack itemStack, final World world, final EntityPlayer entityPlayer) {
		...
		if (entityPlayer.capabilities.isCreativeMode
				|| entityPlayer.inventory.hasItem(ModItems.meteoricThunderstormArrow)) {
	...
	public boolean hitEntity(final ItemStack itemStack, final EntityLivingBase entity1,
		...
		if (Config.meteoricThunderstormBowHitEntityEffect) {
			entity1.setFire(9);
		}
