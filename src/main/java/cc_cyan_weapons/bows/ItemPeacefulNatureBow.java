...
import cc_cyan_weapons.entity.EntityPeacefulNatureArrow;
...
public class ItemPeacefulNatureBow extends ModBow {
	public float arrowDamage;

	public ItemPeacefulNatureBow(final String name, final int maxDamage, final float arrowDamage) {
	...
	public void onPlayerStoppedUsing(final ItemStack itemStack, final World world, final EntityPlayer entityPlayer,
		...
		if (flag || entityPlayer.inventory.hasItem(ModItems.peacefulNatureArrow)) {
			float a = j / 20.0F;
			a = (a * a + a * 2.0F) / 3.0F;
			...
			final EntityPeacefulNatureArrow entityarrow = new EntityPeacefulNatureArrow(world, entityPlayer, a * 2.0F);
			...
			if (flag) {
				...
				entityPlayer.inventory.consumeInventoryItem(ModItems.peacefulNatureArrow);
			}
			if (!world.isRemote) {
	...
	public ItemStack onItemRightClick(final ItemStack itemStack, final World world, final EntityPlayer entityPlayer) {
		...
		if (entityPlayer.capabilities.isCreativeMode || entityPlayer.inventory.hasItem(ModItems.peacefulNatureArrow)) {
	...
	public boolean hitEntity(final ItemStack itemStack, final EntityLivingBase entity1,
		...
		if (Config.peacefulNatureBowHitEntityEffect) {
