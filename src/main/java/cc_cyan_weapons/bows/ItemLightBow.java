...
import cc_cyan_weapons.entity.EntityLightArrow;
...
public class ItemLightBow extends ModBow {
	public float arrowDamage;

	public ItemLightBow(final String name, final int maxDamage, final float arrowDamage) {
	...
	public void onPlayerStoppedUsing(final ItemStack itemStack, final World world, final EntityPlayer entityPlayer,
		...
		if (flag || entityPlayer.inventory.hasItem(ModItems.lightArrow)) {
			float a = j / 20.0F;
			a = (a * a + a * 2.0F) / 3.0F;
			...
			final EntityLightArrow entityarrow = new EntityLightArrow(world, entityPlayer, a * 2.0F);
			...
			if (flag) {
				...
				entityPlayer.inventory.consumeInventoryItem(ModItems.lightArrow);
			}
			if (!world.isRemote) {
	...
	public ItemStack onItemRightClick(final ItemStack itemStack, final World world, final EntityPlayer entityPlayer) {
		...
		if (entityPlayer.capabilities.isCreativeMode || entityPlayer.inventory.hasItem(ModItems.lightArrow)) {
	...
	public boolean hitEntity(final ItemStack itemStack, final EntityLivingBase entity1,
		...
		if (Config.lightBowHitEntityEffect) {
