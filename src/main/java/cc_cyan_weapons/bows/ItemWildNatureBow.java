...
import cc_cyan_weapons.entity.EntityWildNatureArrow;
...
public class ItemWildNatureBow extends ModBow {
	public float arrowDamage;

	public ItemWildNatureBow(final String name, final int maxDamage, final float arrowDamage) {
	...
	public void onPlayerStoppedUsing(final ItemStack itemStack, final World world, final EntityPlayer entityPlayer,
		...
		if (flag || entityPlayer.inventory.hasItem(ModItems.wildNatureArrow)) {
			float a = j / 20.0F;
			a = (a * a + a * 2.0F) / 3.0F;
			...
			final EntityWildNatureArrow entityarrow = new EntityWildNatureArrow(world, entityPlayer, a * 2.0F);
			...
			if (flag) {
				...
				entityPlayer.inventory.consumeInventoryItem(ModItems.wildNatureArrow);
			}
			if (!world.isRemote) {
	...
	public ItemStack onItemRightClick(final ItemStack itemStack, final World world, final EntityPlayer entityPlayer) {
		...
		if (entityPlayer.capabilities.isCreativeMode || entityPlayer.inventory.hasItem(ModItems.wildNatureArrow)) {
	...
	public boolean hitEntity(final ItemStack itemStack, final EntityLivingBase entity1,
		...
		if (Config.wildNatureBowHitEntityEffect) {
